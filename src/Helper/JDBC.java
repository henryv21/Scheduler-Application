...
public abstract class JDBC {
    ...
    private static final String location = "//34.125.65.40/";
    ...
    public static ObservableList<Appointment> searchAppointments(String search) throws SQLException {
        String sql1 = "SELECT Customer_ID FROM customers WHERE Customer_Name LIKE ?";
        PreparedStatement ps1 = JDBC.connection.prepareStatement(sql1);
        ps1.setString(1, search + '%');
        ResultSet rs1 = ps1.executeQuery();
        rs1.next();
        int appointmentCustomer2 = rs1.getInt("Customer_ID");

        String sql2 = "SELECT * FROM appointments WHERE Customer_ID = ?";
        ObservableList<Appointment> appointments = FXCollections.observableArrayList();
        appointments.clear();

        PreparedStatement ps2 = JDBC.connection.prepareStatement(sql2);
        ps2.setString(1,Integer.toString(appointmentCustomer2));
        ...
        if (rs2.next()) {
            ...
            Timestamp appointmentStart = rs2.getTimestamp("Start");
            Timestamp appointmentEnd = rs2.getTimestamp("End");
            int appointmentCustomer = rs2.getInt("Customer_ID");
            int appointmentUser = rs2.getInt("User_ID");
            int appointmentContact = rs2.getInt("Contact_ID");
            Timestamp start = Helper.toLocal(appointmentStart);
            Timestamp end = Helper.toLocal(appointmentEnd);
            Appointment appointment = new Appointment(appointmentId, appointmentTitle, appointmentDescription,
                    appointmentLocation, appointmentType, start,
        ...
        JOptionPane.showMessageDialog(null, "Name/Appointments not Found");
    }
        return appointments;
    }
